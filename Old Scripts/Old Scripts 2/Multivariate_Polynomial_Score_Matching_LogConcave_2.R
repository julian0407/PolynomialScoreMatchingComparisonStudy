...
# with optional grid/data-based log-concavity enforcement.
#
# API:
...
# TODO:
# Convention:
#   The model score is r(x) = - grad log f(x).
#   Hence log-concavity of f is equivalent to convexity of the
#   fitted potential whose gradient is the score.
# ============================================================
...
# ------------------------------------------------------------
# (4) Hessian design for log-concavity checks/penalties
# ------------------------------------------------------------
...
make_logconcavity_points_mv_basic <- function(z,
                                              method = c("grid", "data"),
                                              grid_size = 5L,
                                              box_expand = 0,
                                              max_points = 500L,
  ...
  if (!is.numeric(grid_size) || length(grid_size) != 1L || grid_size < 2L) {
    stop("grid_size must be an integer >= 2 when log_concave = TRUE and lc_method = 'grid'.")
  }
...
penalized_sm_objective_mv_basic <- function(theta,
  ...
  pen <- penalty * mean(lc_diag$violations^2)
  base_obj + pen
}
...
fit_score_matching_mv_basic <- function(x,
  ...
  if (isTRUE(log_concave)) {
    lc_points <- make_logconcavity_points_mv_basic(
      z = z,
      method = lc_method,
