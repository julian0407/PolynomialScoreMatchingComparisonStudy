...
metric_neg_loglik <- function(x_test, fit, family, method, predict_args = list()) {
  logdens <- do.call(
...
evaluate_requested_metrics <- function(metrics,
  ...
  for (met in metrics) {
    if (met == "negloglik") {
      out[[met]] <- metric_neg_loglik(
        x_test, fit, family, method, density_predict_args
      )
    } else if (met == "kl") {
      out[[met]] <- metric_kl_mc(
        x_test, fit, family, method, true_logdensity, density_predict_args
      )
