# ============================================================
# BiasVariance_Score_Clean_patched.R
# Standardized bias-variance analysis for score estimators
# ============================================================
...
run_bias_variance_score_one <- function(n,
  ...
  for (b in seq_len(B)) {
    ...
    fit_time <- system.time({
      fit <- tryCatch(
        do.call(
          ...
          c(
            list(x = x_train, family = family, method = estimator_spec$method, smoothed = estimator_spec$smoothed),
    ...
    score_time <- system.time({
      score_hat <- tryCatch(
        do.call(
          ...
          c(
            list(newx = eval_grid_use, fit = fit, family = family, method = estimator_spec$method),
    ...
    if (!is.null(score_hat)) score_hat <- tryCatch(as_score_matrix(score_hat, n_expected = n_eval), error = function(e) NULL)
    ...
    base_diag$condition_number <- diags$condition_number %||% NA_real_
    diag_rows[[b]] <- base_diag
  }
...
summarize_bias_variance_score_run <- function(obj,
                                              ...
                                              trim_alpha = 0.05) {
  if (!inherits(obj, "score_bias_variance_run")) stop("obj must be of class 'score_bias_variance_run'.")
  bias_center <- match.arg(bias_center)
  if (!is.numeric(trim_alpha) || length(trim_alpha) != 1L || !is.finite(trim_alpha) || trim_alpha < 0 || trim_alpha >= 0.5) {
  ...
  integrate_vec <- function(y) {
    if (obj$family == "univariate" && isTRUE(integrate)) trapz_vec(as.numeric(obj$eval_grid), y) else safe_mean_local(y)
  }
...
run_bias_variance_score_benchmark <- function(sample_sizes,
  ...
  for (spec in estimator_specs) {
    ...
    for (n in sample_sizes) {
      ...
      summ <- summarize_bias_variance_score_run(obj, bias_center = "all", trim_alpha = 0.05)
      runs[[counter]] <- obj
  ...
  if (isTRUE(save)) {
    ...
    out$settings$save_path <- save_path
  }
...
subset_score_bias_variance_benchmark <- function(obj,
  ...
  if (!inherits(obj, "score_bias_variance_benchmark")) stop("obj must be of class 'score_bias_variance_benchmark'.")
  ...
  out$summary <- filter_benchmark_df_by_method(out$summary, keep_method_labels, drop_method_labels, keep_methods, drop_methods)
  out$diagnostics <- filter_benchmark_df_by_n(out$diagnostics, keep_n = keep_n, drop_n = drop_n)
  out$diagnostics <- filter_benchmark_df_by_method(out$diagnostics, keep_method_labels, drop_method_labels, keep_methods, drop_methods)
  out$runs <- Filter(keep_fun, out$runs)
  if (isTRUE(update_settings)) out$settings$sample_sizes <- sort(unique(out$summary$n))
  out
}
...
aggregate_score_bias_variance_benchmark <- function(obj,
  ...
  df <- filter_benchmark_df_by_method(df, keep_method_labels, drop_method_labels, keep_methods, drop_methods)
  miss <- setdiff(value_cols, names(df))
  if (length(miss) > 0L) stop(sprintf("Requested columns not found: %s", paste(miss, collapse = ", ")))
  df[, c("method_label", "method", "n", value_cols), drop = FALSE]
}

plot_score_bias_variance <- function(obj,
                                     metric = c("integrated_bias2", "integrated_variance", "integrated_mse", "integrated_bias2_median",
                                                "integrated_bias2_trim"),
                                     keep_n = NULL,
                                     drop_n = NULL,
                                     keep_method_labels = NULL,
                                     drop_method_labels = NULL,
  ...
  if (nrow(df) == 0L) stop("No rows left after applying the sample-size filter.")
  ...
  p <- apply_optional_log_scale(p, df$n, axis = "x", requested = log_x)
  p <- apply_optional_log_scale(p, df[[metric]], axis = "y", requested = log_y)
  p
}
