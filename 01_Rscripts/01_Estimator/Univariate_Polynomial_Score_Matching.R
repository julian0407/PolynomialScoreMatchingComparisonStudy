# ============================================================
# Univariate_Polynomial_Score_Matching.R
#
# API:
#   - fit_score_matching_univariate
#   - predict_logdensity_sm_1d
#   - predict_density_sm_1d
#   - predict_score_univariate
#
# Convention:
#   score r(x) = (- log f(x))'
# ============================================================
...
build_K_and_l_univariate <- function(
    ...
    h_prime = function(z) rep(0, length(z))
      ...
      # Use column scaling for numerical stability
      # as big m can be very unstable for exponentials
      col_scale_raw <- apply(A, 2, function(v) max(stats::sd(v), 1e-12))
      S_raw <- matrix(col_scale_raw, nrow = m, ncol = m)
      d <- sqrt(pmax(diag(S_raw), 1e-12))
      scale_mat <- outer(d, d, "*")
      scale_vec <- as.vector(scale_mat)
      ...
      # Return K, l and the column scaling vector
...
get_score_polynomial_coefficients_univariate <- function(fit) {
  ...
  # Calculate coefficients of score by summing up related entries in G (with same exponent)
  for (i in seq_len(m)) {
    for (j in seq_len(m)) {
      deg <- i + j - 1L
...
# Integrate score s'(x) to receive s(x)
...
# Build finite search interval around the standardized training sample.
# Goal: Finite interval that contains almost the entire mass
...
# We will use log(int(exp(log[f(z)]-shift)) =  log(int(exp(log[f(z)]))  - shift
find_mode_logdensity_z_univariate <- function(fit,
  ...
  vals <- f_log(grid)
  # Get rough maxima of mode
  idx <- which.max(vals)
  z0 <- grid[idx]
  
  # Get more precise maxima in intervall of grid around rough maxima
...
compute_log_normalizer_z_univariate <- function(fit,
  ...
  # Define integrand with mode shift
  ...
  # Approximate integral over finite interval 
  integ <- tryCatch(
    stats::integrate(
      f = integrand,
      lower = interval[1L],
  ...
  # Get log(normalizing_constant) and normalizing_constant + diagnostics
  list(
...
predict_logdensity_z_univariate <- function(z,
  ...
  # get unnormalized logdensity and normalizing constant
  ...
  # return NA values if error in integral
...
# Get density from log density on z-scale
