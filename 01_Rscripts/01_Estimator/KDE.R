...
fit_kde_1d <- function(x,
  ...
  # Determine bandwidth based on sample x and selected bandwidth type bw
  bw_value <- switch(
    bw,
    SJ   = stats::bw.SJ(x),
...
fit_kde_mv <- function(x,
  ...
  # Ridge regularization: ensure that the bandwidth matrix H is positive definite.
  # This is required for inversion and for evaluating the multivariate Gaussian kernel.
