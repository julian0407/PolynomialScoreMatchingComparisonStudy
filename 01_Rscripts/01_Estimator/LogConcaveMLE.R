...
fit_logconcave_mle_mv <- function(x,
  ...
  Ahat <- if (isTRUE(smoothed)) LogConcDEAD::hatA(fit) else NULL
...
predict_density_logconcave_mv <- function(newx,
  ...
  if (isTRUE(smoothed)) {
    Ahat <- fit$Ahat
    if (is.null(Ahat)) Ahat <- LogConcDEAD::hatA(fit$fit)
    
    return(as.numeric(LogConcDEAD::dslcd(newx, fit$fit, A = Ahat)))
  }
}
