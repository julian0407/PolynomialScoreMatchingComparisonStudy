...
#     NEU: Score-Loss ist jetzt der direkte Score-Loss
#          auf unabhängigem Testsample
# ============================================================
...
res_gumbel <- run_L1_and_density_panels(
  ...
  ns = res_gumbel <- run_L1_and_density_panels(
  name = "Gumbel(0,1) -- log-concave, misspecified",
  ns = c(200, 500, 1000, 2000, 5000, 10000, 20000, 100000, 500000, 1000000),
  R  = 20,
  m  = 3,
...
for (j in seq_along(ms)) {
  ...
  cat("\n============================\n")
  cat("Running Gumbel test for m =", mj, "\n")
  cat("============================\n")
...
n_target <- max(ns_default)

l1_last <- sapply(res_gumbel_by_m, function(res) {
  idx <- which(res$ns == n_target)
  median(res$errs[idx, ], na.rm = TRUE)
})

plot(ms, l1_last, type = "b", pch = 19,
     xlab = "m", ylab = paste("Median L1 at n =", n_target),
     main = "Gumbel: L1 vs m")
grid()

score_last <- sapply(res_gumbel_by_m, function(res) {
  idx <- which(res$ns == n_target)
  median(res$score_losses[idx, ], na.rm = TRUE)
})

plot(ms, score_last, type = "b", pch = 19,
     xlab = "m", ylab = paste("Median Score-Loss at n =", n_target),
     main = "Gumbel: Score-Loss vs m")
grid()