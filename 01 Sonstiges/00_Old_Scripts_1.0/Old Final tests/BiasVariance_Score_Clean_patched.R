...
summarize_bias_variance_score_run <- function(obj,
  ...
  for (j in seq_len(n_eval)) {
    for (k in seq_len(d)) {
      ...
      mean_hat[j, k] <- safe_mean(vals)
      median_hat[j, k] <- safe_median(vals)
      var_hat[j, k] <- safe_var(vals)
  ...
  summary_df <- data.frame(
    ...
    finite_rep_mean = safe_mean(as.numeric(n_finite_mat) / B),
...
subset_score_bias_variance_benchmark <- function(obj,
  ...
  keep_fun <- function(n) {
    out <- TRUE
    if (!is.null(filt$keep_n)) out <- out && (n %in% filt$keep_n)
    if (!is.null(filt$drop_n)) out <- out && !(n %in% filt$drop_n)
    out
  }
  ...
  out$runs <- Filter(function(run) keep_fun(run$n), out$runs)
