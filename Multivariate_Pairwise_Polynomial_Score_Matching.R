...
# TODO: Refer to these code parts
...
# ------------------------------------------------------------
# (4) log-concavity helpers
# ------------------------------------------------------------
# ------------------------------------------------------------
# (4a) Hessian design and grid penalize
# ------------------------------------------------------------
...
make_logconcavity_points_mv <- function(z,
  ...
  # Grid size  exponentially increase with increaisng dimension
  # Sample a maximum number of grid points for which log concavety is enforced
...
project_theta_logconcave_m2_mv_basic <- function(theta, basis, d, eps = 1e-8) {
  ...
  for (col in seq_along(basis)) {
    # get basis and corresponding theta value
    b <- basis[[col]]
    val <- theta[col]
    
    
    if (b$type == "uni") {
      # Univariate linear terms in beta
      if (b$r == 1L) {
        beta[b$j] <- val
      } else if (b$r == 2L) {
        # Univariate quadratic terms in Hessian
        H[b$j, b$j] <- H[b$j, b$j] + 2 * val
      }
    } else if (b$r == 1L && b$s == 1L) {
      # Pairwise terms in Hessian
  ...
  # TODO: Remove?
  # Ensure symmetry of Hessian
  H <- 0.5 * (H + t(H))
  # derive eigenvalues and eigenvectors
  ee <- eigen(H, symmetric = TRUE)
  # Cut off eigenvalues less than zero/eps
  raw_eigenvalues <- ee$values
  ee$values <- pmax(ee$values, eps)
  # Project original Hessian to positive definite Hessian
  H_proj <- ee$vectors %*% diag(ee$values, d, d) %*% t(ee$vectors)
  
  # Initialize numeric vector for projected theta
  theta_proj <- numeric(length(theta))
  # TODO: Why this backtransformation?
  for (col in seq_along(basis)) {
    b <- basis[[col]]
    if (b$type == "uni") {
  ...
  # Return original and projected theta and H and meta data
  list(
    theta = theta_proj,
    beta = beta,
    H_raw = H,
    ...
    projected_eigenvalues = pmax(raw_eigenvalues, eps)
  )
}
...
fit_score_matching_mv_basic <- function(x,
  ...
  # TODO: Check with theory of Drton
  # Add ridge term to K
  ...
  if (isTRUE(log_concave)) {
    if (identical(lc_method, "m2")) {
      # if "m2" project Hessian and theta
      ...
      # Save projected theta and "m2" details
      theta <- as.numeric(m2_proj$theta)
      m2_details <- list(
        beta = m2_proj$beta,
        H_raw = m2_proj$H_raw,
        H = m2_proj$H,
        raw_eigenvalues = m2_proj$raw_eigenvalues,
        projected_eigenvalues = m2_proj$projected_eigenvalues,
        min_eigenvalue = min(eigen(m2_proj$H, symmetric = TRUE, only.values = TRUE)$values),
        eps = lc_m2_eps
      )
      # Save diagnostics to check if method indeed provide log-concavetiy
      lc_points <- z
